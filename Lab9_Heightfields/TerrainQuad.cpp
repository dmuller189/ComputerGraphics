...
void TerrainQuad::draw(const QMatrix4x4& world, const QMatrix4x4& view, const QMatrix4x4& projection)
{
    ...
    //  - After seeing the initial heightmap by querying in C++ the height image
    ...
    //  - Uncomment these lines when youa re ready to move from C++ implementation to
    ...
    for (int s = 0; s < numStrips_; ++s) {
        //   Draw the correct number of triangle strips using glDrawElements
        glDrawElements(GL_TRIANGLE_STRIP, 3 * numTris_ , GL_UNSIGNED_INT, 0);
    }
